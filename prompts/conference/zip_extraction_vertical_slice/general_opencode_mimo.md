...
- Write exactly one output file: `runs/conference/zip_extraction_vertical_slice/general_opencode_mimo.md`. The bounded runner persists your final response there; do not create sibling output files.
...
Task:
Independently review implementability and user-workflow completeness. Define the provider/loader/model/UI boundaries, progress events, cancellation semantics, deterministic output naming, Finder reveal behavior, and real integration fixtures. Challenge whether the design genuinely streams rather than buffering full entries. Do not edit files or read other participants.

Output schema:
1. `# Conference Participant Output: zip_extraction_vertical_slice - general_opencode_mimo`
2. `## Boundary Check`
