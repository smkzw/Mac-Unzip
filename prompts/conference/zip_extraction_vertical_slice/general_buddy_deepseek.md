...
- Write exactly one output file: `runs/conference/zip_extraction_vertical_slice/general_buddy_deepseek.md`. The bounded runner persists your final response there; do not create sibling output files.
...
Task:
Perform an adversarial security and reliability design review of the proposed extraction slice. Build the wished-for API from the current source, then attack path traversal, symlink races, duplicate paths, case/Unicode collisions, empty directories, oversized entries, cancellation between fsync and rename, and pre-existing destination content. Return a prioritized TDD plan. Do not edit files or read other participants.

Output schema:
1. `# Conference Participant Output: zip_extraction_vertical_slice - general_buddy_deepseek`
2. `## Boundary Check`
